...
mcsea_timepoint <- readRDS(file = file.path("output", "data_dmr", "mcsea", "dmr_hMSC_timepoint.RDS"))
...
run_camera <- function(fit, coefficient) {
  ...
  camera_list <- list("promoters" = camera_promoters,
                      "genes" = camera_genes,
                      "CGI" = camera_CGI)
...
dmr_camera <- purrr::map(1:ncol(fit_contrasts$contrasts),
                         \(x) run_camera(fit_contrasts, x))

names(dmr_camera) <- colnames(dmr_camera)

saveRDS(dmr_camera,
        file.path('output', 'data_dmr', 'camera', "camera_all.RDS"))